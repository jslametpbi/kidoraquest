...
const Kidora = (() => {
  ...
  function defaultDb() {
    ...
    const childId = 'u-child-sample';
    const teacherId = 'u-teacher-sample';
    const parentId = 'u-parent-sample';
    return {
      users: [
        ...
        userRecord({ id: parentId, name: 'Parent Guardian', email: 'parent@kidora.local', password: 'parent2026', role: 'parent', approved: true, childId, avatar: '👪' }),
        userRecord({ id: 'u-admin-core', name: 'KIDORA Admin', email: 'admin@kidora.local', password: uid('admin-pass'), role: 'admin', approved: true, avatar: '🛡️' })
      ],
  ...
  function renderAuth() {
    $('#app').innerHTML = `
      ...
      <main class="app-shell auth-page fit-auth">
        <section class="brand-panel compact-landing">
          <div class="logo-row">
          ...
          <p class="hero-sub">A one-screen game-based English world where young learners unlock quests through listening, speaking, reading, writing, drawing, voice evidence, and teacher-controlled AI support.</p>
          <div class="pill-row">
            <span class="pill">SoundQuest AI</span><span class="pill">EchoMirror AI</span><span class="pill">GlyphPath AI</span><span class="pill">StoryForge AI</span><span class="pill">Admin-Controlled</span>
          </div>
          <div class="world-preview compact-world">
            ${ZONES.slice(0,4).map(z => `<div class="world-tile"><span class="emoji">${z.emoji}</span><h3>${z.zone}</h3><p>${z.name}</p></div>`).join('')}
          </div>
          <footer class="landing-copyright">© 2026 KIDORA Quest. Copyright Dr. Joko Slamet. All rights reserved.</footer>
        </section>
        <aside class="auth-panel compact-login">
          <div class="tabbar"><button class="${state.authMode === 'login' ? 'active' : ''}" onclick="Kidora.setAuthMode('login')">Learner / Teacher / Parent</button><button class="${state.authMode === 'register' ? 'active' : ''}" onclick="Kidora.setAuthMode('register')">Register</button></div>
          ${state.authMode === 'login' ? loginForm() : registerForm()}
          <div class="admin-pin-card">
            <h3>Admin Access</h3>
            <form class="form-grid" onsubmit="Kidora.adminPinLogin(event)">
              <div class="field"><label>Admin PIN</label><input class="input" id="adminPinInput" type="password" inputmode="numeric" placeholder="Enter PIN" required /></div>
              <button class="btn red" type="submit">Open Admin Console</button>
            </form>
          </div>
        </aside>
      </main>`;
  ...
  function loginForm() {
    return `<form class="form-grid" onsubmit="Kidora.login(event)">
      <div class="field"><label>Email</label><input class="input" id="loginEmail" type="email" required placeholder="Enter registered email" /></div>
      <div class="field"><label>Password</label><input class="input" id="loginPassword" type="password" required placeholder="Enter password" /></div>
  ...
  function login(event) {
    ...
    const user = state.db.users.find(u => u.role !== 'admin' && u.email.toLowerCase() === email && u.password === password);
    if (!user) return toast('Login failed. Check email and password. Admin uses PIN access.');
  ...
  function adminPinLogin(event) {
    event.preventDefault();
    const pin = $('#adminPinInput').value.trim();
    if (pin !== state.db.settings.adminPin) return toast('Invalid admin PIN.');
    let admin = state.db.users.find(u => u.role === 'admin');
    if (!admin) {
      admin = userRecord({ name: 'KIDORA Admin', email: 'admin@kidora.local', password: uid('admin-pass'), role: 'admin', approved: true, avatar: '🛡️' });
      state.db.users.push(admin);
      save();
    }
    state.user = admin;
    state.page = 'dashboard';
    sessionStorage.setItem('kidora.currentUser', admin.id);
    render();
  }
  ...
  function getNavItems(role) {
    if (role === 'child') return [
      ...
      { key: 'listening', icon: '🎧', label: 'SoundQuest', subtitle: 'Play, stop, repeat, and answer AI-guided listening missions.' },
      { key: 'speaking', icon: '🎙️', label: 'EchoMirror', subtitle: 'Record, stop, replay, and save voice evidence.' },
      { key: 'reading', icon: '🌱', label: 'GlyphPath', subtitle: 'Read words, hear pronunciation, match pictures, and grow reading paths.' },
      { key: 'writing', icon: '✍️', label: 'StoryForge', subtitle: 'Trace, draw, write, save, and publish story pages.' },
  ...
  function childRoute() {
    ...
    if (['listening','speaking','reading','writing'].includes(state.page)) return renderSkillStudio(state.page);
  ...
  function afterRender() {
    if (state.user?.role === 'child' && ((state.page === 'quest' && state.activeQuest && STEPS[state.questStep] === 'Create') || state.page === 'writing')) initCanvas();
  }
  ...
  function getActiveLearningQuest() {
    return state.activeQuest || state.db.quests.find(q => q.published) || state.db.quests[0];
  }

  function renderSkillStudio(skill) {
    const q = getActiveLearningQuest();
    if (!q) return `<section class="panel"><div class="empty">No quest content available. Ask the teacher/admin to publish a quest.</div></section>`;
    const targetWord = q.tasks.hear.target;
    const targetSentence = q.tasks.say.target;
    const zone = ZONES.find(z => z.key === skill) || ZONES[0];
    const skillMap = {
      listening: {
        title: 'SoundQuest AI Studio',
        intro: 'Children practise listening through play, stop, repeat, picture clues, commands, and sound-word matching.',
        body: `<div class="studio-board sound-board"><div class="report-box"><strong>AI Listening Target</strong><br/>${esc(targetWord)}<br/><span class="mini-note">Quest source: ${esc(q.title)}</span></div>
          <div class="control-row"><button class="btn alt" onclick="Kidora.speak('${esc(targetWord)}')">▶ Play Word</button><button class="btn ghost" onclick="Kidora.speak('${esc(q.tasks.hear.instruction)}')">▶ Play Instruction</button><button class="btn red" onclick="Kidora.stopAudio()">⏹ Stop Audio</button></div>
          <div class="options">${shuffle(q.vocab).slice(0,4).map(v => `<button class="option-card" onclick="Kidora.chooseHear('${v.word}', '${targetWord}')"><span class="emoji">${v.emoji}</span>${esc(v.word)}</button>`).join('')}</div>
          <button class="btn green" onclick="Kidora.saveSkillArtifact('listening')">Save Listening Evidence</button></div>`
      },
      speaking: {
        title: 'EchoMirror AI Studio',
        intro: 'Children hear a target, record their own voice, stop recording, replay it, clear it, and save speaking evidence.',
        body: `<div class="studio-board speak-board"><div class="report-box"><strong>Voice Target</strong><br/>${esc(targetSentence)}</div>
          <div class="control-row"><button class="btn alt" onclick="Kidora.speak('${esc(targetSentence)}')">▶ Play Target</button><button class="btn red" onclick="Kidora.stopAudio()">⏹ Stop Audio</button><button class="btn green" onclick="Kidora.startRecording()">🎙 Start Record</button><button class="btn red" onclick="Kidora.stopRecording()">⏹ Stop Record</button><button class="btn ghost" onclick="Kidora.playRecordedVoice()">▶ Play My Voice</button><button class="btn ghost" onclick="Kidora.clearRecording()">Clear Voice</button></div>
          <div id="recordingStatus" class="mini-note">Press Start Record, allow microphone permission, speak the target sentence, then press Stop Record.</div>
          ${state.audioData ? `<audio class="audio-preview" controls src="${state.audioData}"></audio>` : '<div class="empty">No recorded voice yet.</div>'}
          <button class="btn green" onclick="Kidora.saveSkillArtifact('speaking')">Save Voice Evidence</button></div>`
      },
      reading: {
        title: 'GlyphPath AI Studio',
        intro: 'Children hear the word, stop audio, read the glyph path, match picture-word meaning, and save reading evidence.',
        body: `<div class="studio-board read-board"><div class="report-box"><strong>Read the Glyph Path</strong><br/><span style="font-size:48px;letter-spacing:8px">${esc(targetWord)}</span></div>
          <div class="control-row"><button class="btn alt" onclick="Kidora.speak('${esc(targetWord)}')">▶ Hear Word</button><button class="btn ghost" onclick="Kidora.speak('Read the word ${esc(targetWord)} and choose the matching picture.')">▶ Hear Reading Clue</button><button class="btn red" onclick="Kidora.stopAudio()">⏹ Stop Audio</button></div>
          <div class="options">${shuffle(q.vocab).slice(0,4).map(v => `<button class="option-card" onclick="Kidora.chooseRead('${v.word}', '${targetWord}')"><span class="emoji">${v.emoji}</span>${esc(v.word)}</button>`).join('')}</div>
          <button class="btn green" onclick="Kidora.saveSkillArtifact('reading')">Save Reading Evidence</button></div>`
      },
      writing: {
        title: 'StoryForge AI Studio',
        intro: 'Children trace, draw, copy, type, save canvas evidence, and publish a multimodal storybook page.',
        body: `<div class="studio-board write-board"><div class="report-box"><strong>Writing Target</strong><br/>${esc(targetSentence)}</div>
          <div class="canvas-wrap"><canvas id="drawCanvas" width="860" height="360"></canvas></div>
          <div class="control-row"><button class="btn ghost" onclick="Kidora.clearCanvas()">Clear Canvas</button><button class="btn" onclick="Kidora.saveCanvas()">Save Drawing</button><button class="btn alt" onclick="Kidora.speak('${esc(targetSentence)}')">▶ Read Sentence</button><button class="btn red" onclick="Kidora.stopAudio()">⏹ Stop Audio</button></div>
          <div class="field"><label>Copy, trace, or write the sentence</label><input id="writingInput" class="input" value="${esc(targetSentence)}" /></div>
          <button class="btn green" onclick="Kidora.saveSkillArtifact('writing')">Save StoryForge Evidence</button></div>`
      }
    };
    const item = skillMap[skill];
    return `<div class="grid"><section class="panel skill-hero" style="--zone:${zone.color}"><div class="section-title"><div><h3>${zone.emoji} ${item.title}</h3><p>${item.intro}</p></div><button class="btn alt" onclick="Kidora.startRecommendedQuest()">Open Full Quest</button></div><div class="skill-strip"><span>AI-guided</span><span>Play / Stop</span><span>Evidence-based</span><span>Portfolio-ready</span></div></section><section class="panel">${item.body}</section><section class="panel"><h3>Skill Integration</h3><div class="grid four"><div class="mini-skill">🎧 Listen</div><div class="mini-skill">🎙 Speak</div><div class="mini-skill">🌱 Read</div><div class="mini-skill">✍️ Write</div></div><p class="mini-note">This studio is connected to the same quest vocabulary and portfolio system, so every activity contributes to GrowthTrail analytics.</p></section></div>`;
  }
  ...
  function activityForStep(q, step) {
    ...
    if (step === 'Hear') {
      ...
      return `<div>
        ...
        <button class="btn red" onclick="Kidora.stopAudio()">⏹ Stop Audio</button>
    ...
    if (step === 'Say') {
      return `<div>
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:14px">
          ...
          <button class="btn red" onclick="Kidora.stopRecording()">⏹ Stop Recording</button>
          <button class="btn ghost" onclick="Kidora.playRecordedVoice()">▶ Play My Voice</button>
          <button class="btn ghost" onclick="Kidora.clearRecording()">Clear Voice</button>
        </div>
    ...
    if (step === 'Read') {
      ...
      return `<div>
        ...
        <div class="control-row"><button class="btn alt" onclick="Kidora.speak('${esc(target)}')">▶ Hear Word</button><button class="btn red" onclick="Kidora.stopAudio()">⏹ Stop Audio</button></div>
    ...
    if (step === 'Create') {
      return `<div>
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:10px">
          ...
          <button class="btn red small" onclick="Kidora.stopAudio()">⏹ Stop Audio</button>
        </div>
  ...
  function chooseHear(word, target) {
    ...
    const msg = AI.feedback({ skill: 'listening', correct, target });
    const fb = $('#aiFeedback');
  ...
  function chooseRead(word, target) {
    ...
    const msg = AI.feedback({ skill: 'reading', correct, target });
    const fb = $('#aiFeedback');
    if (fb) fb.textContent = msg;
    else toast(msg);
  ...
  function renderAdminDashboard() {
    return `<div class="grid">
      <section class="panel"><div class="section-title"><div><h3>Admin Control Center</h3><p>PIN-secured control for users, AI safety, quests, skill modules, portfolio evidence, audio recording, analytics, and deployment settings.</p></div></div><div class="grid four"><div class="stat-card"><div class="label">Users</div><div class="value">${state.db.users.length}</div></div><div class="stat-card"><div class="label">Pending Safety</div><div class="value">${state.db.safetyQueue.filter(x => x.status !== 'Approved').length}</div></div><div class="stat-card"><div class="label">Published Quests</div><div class="value">${state.db.quests.filter(q => q.published).length}</div></div><div class="stat-card"><div class="label">Analytics</div><div class="value">${state.db.analytics.length}</div></div></div></section>
      <section class="panel"><h3>Direct Admin Actions</h3><div class="grid four"><button class="btn" onclick="Kidora.go('users')">Manage Users</button><button class="btn alt" onclick="Kidora.go('content')">Control Content</button><button class="btn green" onclick="Kidora.go('safety')">Review AI Quests</button><button class="btn ghost" onclick="Kidora.go('analytics')">Export Analytics</button></div></section>
      <section class="panel"><h3>Integrated Feature Health</h3><div class="grid three">${['One-screen landing','Admin PIN access','SoundQuest play/stop','EchoMirror recording','GlyphPath read-aloud','StoryForge canvas','Living Storybook','Teacher AI generator','Parent reports','Research export','PWA offline cache','Admin settings'].map(x => `<div class="card feature-ok"><h3>✅ ${esc(x)}</h3><p>Connected to local database, role routing, evidence storage, and analytics events.</p></div>`).join('')}</div></section>
    </div>`;
  }

  function renderAdminUsers() {
    return `<div class="grid">
      <section class="panel"><div class="section-title"><div><h3>User Management</h3><p>Approve, create, delete, and review role-based access.</p></div></div><div class="table-wrap"><table><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Actions</th></tr></thead><tbody>${state.db.users.map(u => `<tr><td>${u.avatar} ${esc(u.name)}</td><td>${esc(u.email)}</td><td>${esc(u.role)}</td><td>${u.approved ? 'Approved' : 'Pending'}</td><td><button class="btn small ${u.approved ? 'ghost' : 'green'}" onclick="Kidora.toggleApproval('${u.id}')">${u.approved ? 'Set Pending' : 'Approve'}</button> <button class="btn small red" onclick="Kidora.deleteUser('${u.id}')" ${u.role === 'admin' ? 'disabled' : ''}>Delete</button></td></tr>`).join('')}</tbody></table></div></section>
      <section class="panel"><h3>Create User</h3><form class="form-grid" onsubmit="Kidora.adminCreateUser(event)"><div class="field"><label>Name</label><input class="input" id="adminUserName" required /></div><div class="field"><label>Email</label><input class="input" id="adminUserEmail" type="email" required /></div><div class="field"><label>Password</label><input class="input" id="adminUserPassword" required minlength="6" /></div><div class="field"><label>Role</label><select id="adminUserRole"><option value="child">Young Learner</option><option value="teacher">Teacher</option><option value="parent">Parent</option></select></div><button class="btn alt">Create and Approve User</button></form></section>
    </div>`;
  }
  ...
  function adminCreateUser(event) {
    ...
    if (state.db.users.some(u => u.email.toLowerCase() === email)) return toast('Email already exists.');
    state.db.users.push(userRecord({ name, email, password, role, approved: true, avatar: avatarFor(role) }));
    save();
    toast('User created and approved.');
    renderApp();
  }

  function deleteUser(id) {
    const u = state.db.users.find(x => x.id === id);
    if (!u || u.role === 'admin') return toast('Admin account cannot be deleted.');
    if (!confirm(`Delete ${u.name}? This removes local data for this user.`)) return;
  ...
  function resetData() {
    if (!confirm('Reset all local app data in this browser?')) return;
  ...
  function playRecordedVoice() {
    if (!state.audioData) return toast('No recorded voice to play yet.');
    stopAudio();
    state.audioPlayer = new Audio(state.audioData);
    state.audioPlayer.play().catch(() => toast('Browser blocked audio playback. Press the audio control manually.'));
  }
  ...
  function clearRecording() {
    ...
    state.audioData = '';
    state.audioChunks = [];
    toast('Voice recording cleared.');
    renderApp();
  }

  function saveSkillArtifact(skill) {
    if (!state.user || state.user.role !== 'child') return toast('Only learner accounts can save portfolio evidence.');
    const q = getActiveLearningQuest();
    if (!q) return toast('No quest content is available.');
    const targetWord = q.tasks.hear.target;
    const targetSentence = q.tasks.say.target;
    const writingInput = $('#writingInput');
    const sentence = writingInput?.value || targetSentence;
    const artifact = {
      id: uid('art'), questId: q.id, questTitle: `${q.title} – ${skill} studio`, theme: q.theme,
      sentence: skill === 'listening' ? `I listened to ${targetWord}.` : skill === 'reading' ? `I read ${targetWord}.` : sentence,
      drawingData: skill === 'writing' ? state.drawingData : '',
      audioData: skill === 'speaking' ? state.audioData : '',
      words: q.vocab.map(v => v.word), date: now(), skills: [skill]
    };
    state.user.progress.portfolio.unshift(artifact);
    award(skill, skill === 'speaking' ? 12 : 10);
    recordAnalytics(`${skill}_studio_saved`, { questId: q.id, artifactId: artifact.id });
    save();
    toast(`${skill} evidence saved to Living Storybook.`);
    renderApp();
  }
  ...
  function simulateRecording(text = 'Voice evidence') {
  ...
  return {
    init, setAuthMode, login, adminPinLogin, register, go, logout,
    ...
    toggleApproval, adminCreateUser, deleteUser, addContent, deleteQuest, saveSettings, resetData,
    speak, stopAudio, playRecordedVoice, clearRecording, startRecording, stopRecording, simulateRecording, saveSkillArtifact, saveCanvas, clearCanvas
  };
})();
