...
## KIDORA Quest v2 Fixes

This build includes the requested deployment fixes:

- One-screen fit landing page.
- No visible demo account information on the landing page.
- Admin login through PIN only. Default PIN: `2026`.
- Separate working child studios: SoundQuest, EchoMirror, GlyphPath, and StoryForge.
- Working browser speech controls: play and stop audio.
- Working microphone recording through `MediaRecorder` where the browser allows microphone access.
- Speaking module includes start record, stop record, play my voice, clear voice, and save voice evidence.
- Writing module includes canvas drawing, clear canvas, save drawing, read sentence, stop audio, and save story evidence.
- Teacher, parent, and admin dashboards remain integrated with the same local browser database.
- Admin can manage users, content bank, quests, safety review, analytics export, and platform settings.

### Important Browser Note

Voice recording requires HTTPS or localhost. GitHub Pages uses HTTPS, so microphone recording will work after the learner gives browser permission. Some browsers block microphone access in private/incognito mode or when permission is denied.
